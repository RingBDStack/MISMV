...
updated 06/16 
"""
def convert_long_symp(mongo):

    index_patient = 0
    for doc in mongo.find({"symp_hits": {"$exists": True}}).skip(26729):
        hadm_id = doc["hadm_id"]
        #print(hadm_id)
        for symp in doc["symp_hits"]:
            codes = [each for each in symp if len(each) >= 11]
            if codes:
                mongo.update_one({'hadm_id': hadm_id}, {
                                '$addToSet': {'symp_hits_0616': codes}})
        index_patient += 1
        if index_patient % 1000 == 0:
            print(index_patient) 

def tag_three_diseases(cursor, mongo):
    cursor.execute("select distinct(hadm_id) from diagnoses_icd\
        where seq_num = 1 and icd9_code in ('51881', '5849', '41011', '41041');")
    print("SQL executed.")
    for each in cursor.fetchall():
        hadm_id = each[0]
        #print(hadm_id)
...
def get_symp_dict_0616(mongo):

    code_to_hadmid ={}
    index_patient = 0
    for doc in mongo.find({"symp_hits_0616": {"$exists": True}, "tag": {"$exists": True}}):
        hadm_id = doc["hadm_id"]
        #print(hadm_id)
...
generates mimic.symp_tree
"""
def get_symp_tree_0616(code_to_hadmid, mongo_symp_tree_0616):
    for code in code_to_hadmid.keys():
        parent = code[0:-4]
        mongo_symp_tree_0616.insert_one({ "_id": code, "hadm_ids": code_to_hadmid[code], "parent": parent})

"""
diag_to_hadmids: a dictionary that contains diagnose icd9 code and all hadm_ids whose 'diagnoses' field contains the code
"""
def get_diagnose_dict_0616(mongo):

    diag_to_hadmids = {}
    index = 0
    for adm in mongo.find({"diagnoses": {"$exists": True}, "tag": {"$exists": True}}):
        hadm_id = adm['hadm_id']
        for code in adm['diagnoses']:
            if code not in diag_to_hadmids.keys():
                diag_to_hadmids[code] = []
...
if __name__ == '__main__':
    ...
    #convert_long_symp(mongo)
    #tag_three_diseases(cursor, mongo)
    code_to_hadmid = get_symp_dict_0616(mongo)
    dict_path = ""
    with open(dict_path + '/code_to_hadmid_0616.txt','w') as f:
        for i, code in enumerate(code_to_hadmid.keys()):
            #print(str(i) + ': ' + str(code) + '\tadam_ids: ' + str(code_to_hadmid[code]))
            f.write(str(code) + '\t' + str(code_to_hadmid[code]) + '\n')
    mongo_symp_tree_0616 = MongoClient().mimic.symp_tree_0616
    get_symp_tree_0616(code_to_hadmid, mongo_symp_tree_0616)
    """
    diag_to_hadmids = get_diagnose_dict_0616(mongo)
    dict_path = ""
    with open(dict_path + '/diag_to_hadmids_0616.txt','w') as f:
        for i, code in enumerate(diag_to_hadmids.keys()):
            if code:
                #print(code + '\t' + str(diag_to_hadmids[code]))
                f.write(code + '\t' + str(diag_to_hadmids[code]) + '\n')
    mongo_diag_tree_0616 = MongoClient().mimic.diag_tree_0616
    mongo_diagnose_tree(diag_to_hadmids, mongo_diag_tree_0616)
